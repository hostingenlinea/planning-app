...
const Organigram = () => {
  const [areas, setAreas] = useState([]); // Visualmente Áreas
                  ...
                  {areas.map((area) => (
                    <li key={area.id}>
                      <div className="bg-white p-3 rounded-lg shadow-md border-t-4 border-blue-500 w-40 mx-auto hover:shadow-xl transition-shadow relative group z-10">
                        ...
                        <div className="font-bold text-gray-800 text-sm">{area.name}</div>
                        <div className="text-xs text-gray-400 mt-1">{area.teams.length} Equipos</div>
                      </div>
                                ...
                                {team.members && team.members.length > 0 && (
                                    <div className="mt-2 pt-2 border-t border-gray-100 space-y-1">
                                        {team.members.map(tm => (
                                            <button 
                                              key={tm.id} 
                                              onClick={() => handleMemberClick(tm.member.id)}
