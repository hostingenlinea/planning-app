...
import { Cake, Calendar, Gift, MapPin } from 'lucide-react';
...
const Anniversaries = () => {
  ...
  const fetchData = async () => {
    try {
      ...
      // Filtrar solo los que cumplen años este mes
      const birthdays = res.data.filter(m => {
        if (!m.birthDate) return false;
        const d = new Date(m.birthDate);
        return d.getMonth() === currentMonth; // Mismo mes
      }).sort((a, b) => {
    ...
    <div className="p-4 md:p-6 max-w-7xl mx-auto">
      ...
      {loading ? (
        ...
        <div className="text-center p-10 bg-gray-50 rounded-xl border border-dashed">
          <p className="text-gray-400">No hay cumpleaños registrados para este mes.</p>
        </div>
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {members.map(member => {
             ...
             return (
               <div key={member.id} className={`relative bg-white p-6 rounded-2xl shadow-sm border-2 overflow-hidden hover:shadow-lg transition-shadow group
                 ${isToday ? 'border-pink-400 ring-4 ring-pink-50' : 'border-gray-100'}
               `}>
                 {isToday && (
                 ...
                 <div className="flex items-center gap-4">
                   ...
                   <div className="w-16 h-16 rounded-full bg-gray-100 border-2 border-white shadow flex items-center justify-center overflow-hidden shrink-0">
                      {member.photo ? (
                        ...
                        <span className="text-2xl font-bold text-gray-400">{member.firstName[0]}</span>
                      )}
                   </div>
                   ...
                   <div>
                     ...
                     <div className="flex items-center gap-2 mt-2 text-gray-500 text-sm">
                       ...
                       <span>Cumple {getAge(member.birthDate)} años</span>
                     </div>
                     {member.city && (
                 ...
                 <div className="mt-6 flex items-center justify-between bg-gray-50 p-3 rounded-xl">
                   ...
                   <div className="flex-1 text-center text-sm text-gray-600 italic">
                     "¡Que Dios te bendiga mucho en tu día!"
                   </div>
                 </div>
               </div>
             )
