...
import { 
  ...
  Monitor, Trash2, GripVertical, Users, UserPlus, X, CheckCircle 
} from 'lucide-react';
...
const ServiceDetail = () => {
  ...
  const [ministries, setMinistries] = useState([]); // Estructura para saber qué equipos existen
  ...
  const [assigningTeamId, setAssigningTeamId] = useState(null); // ID del equipo que estamos llenando
  ...
  const fetchData = async () => {
    try {
      const [serviceRes, minRes] = await Promise.all([
        ...
        axios.get(`${API_URL}/api/ministries`) // Necesitamos saber los equipos y sus miembros
      ]);
  ...
  const handleAddPlanItem = async (e) => {
    ...
    setNewItem({ title: '', duration: 5, type: 'SONG' });
  ...
  return (
    <div className="p-4 md:p-6 max-w-7xl mx-auto">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        {/* COLUMNA IZQUIERDA: CRONOGRAMA (2/3 del ancho) */}
        <div className="lg:col-span-2 space-y-4">
          ...
          <div className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
             ...
             {isAddingPlan && (
              <form onSubmit={handleAddPlanItem} className="bg-blue-50 p-4 border-b border-blue-100 flex gap-2 items-end">
                <input className="flex-1 p-2 rounded border text-sm" placeholder="Título..." value={newItem.title} onChange={e=>setNewItem({...newItem, title:e.target.value})} autoFocus />
                <input type="number" className="w-20 p-2 rounded border text-sm" placeholder="Min" value={newItem.duration} onChange={e=>setNewItem({...newItem, duration:e.target.value})} />
                <select className="p-2 rounded border text-sm bg-white" value={newItem.type} onChange={e=>setNewItem({...newItem, type:e.target.value})}>
                  <option value="SONG">Canción</option>
                  <option value="PREACHING">Prédica</option>
                  <option value="ANNOUNCEMENT">Anuncio</option>
                </select>
                <button type="submit" className="bg-blue-600 text-white px-3 py-2 rounded text-sm font-bold">OK</button>
              </form>
            )}
              ...
              {service.plans.map((item) => {
                ...
                return (
                  <div key={item.id} className="group flex items-center p-3 hover:bg-gray-50 transition-colors gap-3">
                    ...
                    <div className="p-2 bg-gray-50 rounded-lg">{getTypeIcon(item.type)}</div>
        ...
        {/* COLUMNA DERECHA: EQUIPOS (1/3 del ancho) */}
        <div className="space-y-4">
          ...
          <div className="space-y-4">
              ...
              <div key={ministry.id} className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
                ...
                <div className="divide-y divide-gray-100">
                  {ministry.teams.map(team => {
                    // Filtrar asignaciones para este equipo en este servicio
                    ...
                    return (
                      <div key={team.id} className="p-3">
                        ...
                        {/* SELECTOR DE PERSONAS (Desplegable) */}
                          ...
                          <div className="mt-2 bg-gray-50 p-2 rounded border border-gray-200 animate-fade-in">
                            ...
                            <div className="flex flex-col gap-1 max-h-40 overflow-y-auto">
                              {team.members && team.members.length > 0 ? (
                                ...
                                <p className="text-xs text-red-400">El equipo está vacío. Agrega personas en "Ministerios".</p>
                              )}
                              {/* Botón para añadir a alguien que no sea del equipo (opcional) */}
                              <Link to="/ministries" className="text-xs text-center text-blue-500 hover:underline mt-1 pt-1 border-t">
                                Gestionar Equipo
                              </Link>
                            </div>
                          </div>
            ...
            {ministries.length === 0 && <div className="text-center text-gray-400 text-sm">No hay equipos creados.</div>}
          </div>
        </div>
      </div>
    </div>
